...
public class HttpCrawlerConsumerCommonHttp extends ScheduledPollConsumer {
	...
	private static final Log LOG = LogFactory.getLog(HttpCrawlerConsumerCommonHttp.class);

	public HttpCrawlerConsumerCommonHttp(DefaultEndpoint endpoint, Processor processor) {
	...
	public HttpCrawlerConsumerCommonHttp(Endpoint endpoint, Processor processor,
	...
	protected int poll() throws Exception {
		...
		try {
			...
			HttpMethod method = createInitialRequest(uri);			
			...
			if (status == 200) {
				...
				System.out.println(readFully(method.getResponseBodyAsStream()));
			}
	 ...
	 * @throws IOException
	 */
	protected void processSite(HttpMethod method) throws IOException {
	 ...
	 * @return
	 */
	protected HttpMethod createInitialRequest(URI uri) {
